    ...
    <div class="p-6 flex space-x-2">
        ...
        <div class="flex-1">
            <div class="flex justify-between items-center">
                <div>
                    <span class="text-gray-800">{{ paint.user.name }}</span>
                    <small class="ml-2 text-sm text-gray-600">{{ dayjs(paint.created_at).fromNow() }}</small>
                    <small v-if="paint.created_at !== paint.updated_at" class="text-sm text-gray-600"> &middot;
                        edited</small>
                </div>
            ...
            <p v-else class="mt-4 text-lg text-gray-900">{{ paint.brand }}</p>
        </div>
    </div>
</template>
